...
{                                                                        \
    if (qsim_gen_callbacks) {                                                   \
        /*We need the SUFF size for QSIM*/                                      \
        TCGv_i32 tmp_size = 0, tmp_type = 0;                                        \
        int size;                                                               \
        /*  suffix */                                                           \
        ...
        tmp_type = tcg_const_i32(0);                                            \
        /*Get the addr */                                                       \
        TCGv addr = gen_aa32_addr(s, a32, OPC | s->be_data);                    \
        gen_helper_load_callback_pre(cpu_env, addr, tmp_size, tmp_type);        \
        /*Load it */                                                            \
        tcg_gen_qemu_ld_i32(val, addr, index, OPC | s->be_data);                \
        gen_helper_load_callback_post(cpu_env, addr, tmp_size, tmp_type);       \
        /*Free it up*/                                                          \
        tcg_temp_free_i32(tmp_size);                                            \
        tcg_temp_free_i32(tmp_type);                                            \
        ...
        /* Use the Normal QEMU funktion call */                                 \
...
{                                                                        \
    if (qsim_gen_callbacks) {                                              \
        ...
        gen_helper_store_callback_pre(cpu_env, addr, tmp_size, tmp_type);  \
        /*Store it */                                                      \
        tcg_gen_qemu_st_i32(val, addr, index, OPC | s->be_data);           \
        gen_helper_store_callback_post(cpu_env, addr, tmp_size, tmp_type); \
        /*Free it up*/                                                     \
        tcg_temp_free_i32(tmp_size);                                       \
        tcg_temp_free_i32(tmp_type);                                       \
                            ...
                            int index, TCGMemOp opc)
{
    ...
    if (!IS_USER_ONLY && s->sctlr_b) {
        ...
        if (qsim_gen_callbacks) {
            gen_helper_store_callback_pre(cpu_env, tmp, tmp_size, tmp_type);
        }
        tcg_gen_qemu_st_i64(tmp, addr, index, opc);
        if (qsim_gen_callbacks) {
          gen_helper_store_callback_post(cpu_env, tmp, tmp_size, tmp_type);
        }
